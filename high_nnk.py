...
class High_NNK:
    def __init__(self, config=None, flags=None):
        ...
        self.build_model(tf_utils.augment_data(self.images, self.flags.regularize, self.is_training), self.flags.separate_relu)
    ...
    def network_architecture(self, input_data, scope_name="network", scope_reuse=False):
        ...
        with tf.variable_scope(scope_name, reuse=scope_reuse):
            ...
            # activation_dict[0] = self.dropout_layer(tf_utils.conv2d_basic(input_data, W, b))
            for ii in range(1, self.flags.n_layers + 1):
                ...
                # activation = tf_utils.conv2d_basic(activation_dict[ii - 1], W, b)
    ...
    def network_architecture_v2(self, input_data, scope_name="network", scope_reuse=False):
        activation_dict = {}
        layer_size = self.flags.layer_size
        with tf.variable_scope(scope_name, reuse=scope_reuse):
            W = tf_utils.weight_variable([3, 3, self.IMAGE_SHAPE[2], layer_size], name="W_conv0")
            b = tf_utils.bias_variable([layer_size], name="b_conv0")
            activation_dict[0] = tf_utils.conv2d_basic(input_data, W, b)
            activation_dict[1] = self.dropout_layer(tf.nn.relu(activation_dict[0]))
            for ii in range(1, self.flags.n_layers + 1):
                W = tf_utils.weight_variable([3, 3, layer_size, layer_size], name="W_conv" + str(ii))
                b = tf_utils.bias_variable([layer_size], name="b_conv" + str(ii))
                activation_dict[ii*2] = tf_utils.conv2d_basic(activation_dict[ii*2 - 1], W, b)
                activation_dict[ii*2+1] = self.dropout_layer(tf.nn.relu(activation_dict[ii*2]))
                if (ii + 1) % 2 == 0:  # Pool after every 2 layers of convolution
                    activation_dict[ii*2+1] = tf_utils.max_pool_2x2(activation_dict[ii*2+1])

        return activation_dict

    def build_model(self, input_data, separate_relu):
        scope_name = "network"
        if separate_relu:
            self.net = self.network_architecture_v2(input_data, scope_name=scope_name, scope_reuse=False)
        else:
    ...
    def train_neighbor_search(self, layer, channel, knn_param=1, use_gpu=False, folder_prefix="", save_ann=True):
    
        if channel is None:
            vector = self.net[layer][:,:,:,:]
            print(f'\n\n\n*** Layer: {layer} ***')
        else:
            vector = self.net[layer][:,:,:,channel]
            print(f'\n\n\n*** Layer: {layer} ***')
        ...
        # David: al nom train_neighbor_folder afegir-li la layer i el channel per aixi guardar
        # totes les activacions per cada cas, pero ara no ho faig perque ocupa massa
        # train_neighbor_folder = os.path.join(self.model_output_folder, folder_prefix+'_'+str(layer))
        ...
        for itr in trange(n_batches, desc="Adding train data to ANN"):
            ...
            activation = self.sess.run(vector, feed_dict={self.images: batch_images})
    ...
    def plot_neighbors(self, indices):
        ...
        if data_type == "train":
            ...
            raise EnvironmentError("Unknown calibration save data type: %s" % data_type)
        plot_results_path = os.path.join("plot_images/")
        if not os.path.exists(plot_results_path):
            os.makedirs(plot_results_path)
        knn_layers = sorted(map(int, self.flags.knn_layers.split(",")))
        knn_channels = sorted(map(int, self.flags.knn_channels.split(",")))
        knn_channels.append('All')
        # layer = knn_layers[-1]
        
        neighbor_results_path = 'results'
        if not os.path.exists(neighbor_results_path):
            os.makedirs(neighbor_results_path)
        # file_json = 'results/results_query_' + str(indices).replace('[','').replace(']','').replace(', ','_') + '_layers_' + str(knn_layers).replace('[','').replace(']','').replace(', ','_') + "_channels_" + str(knn_channels).replace('[','').replace(']','').replace(', ','_') + ".json"
        # f_json = open(os.path.join(plot_results_path, file_json), 'w+')
        # f_json = open(file_json, 'w+')
        
        file_csv = os.path.join(neighbor_results_path, 'results_query_' + str(indices).replace('[','').replace(']','').replace(', ','_') + '_layers_' + str(knn_layers).replace('[','').replace(']','').replace(', ','_') + "_channels_" + str(knn_channels).replace('[','').replace(']','').replace(', ','_').replace("'","") + ".csv")
        with open(file_csv, 'w+') as f_csv:
            print('Query,Layer,Channel,Num Neighbors,Num Neighbors Same Class,Num Neighbors Other Classes,Weight Same Class,Weight Same Class k-NN', file=f_csv)
        
        print(self.net)
        stats_per_layer = {}
        for layer in knn_layers:  
            stats_per_channel = {}
            for channel in knn_channels:
                batch_images = test_dataset.images[indices]
                batch_labels = np.argmax(test_dataset.labels[indices], axis=1)
                
                if channel == 'All':
                    neighbor_search = self.train_neighbor_search(layer, None, knn_param=knn_param,
                                                                folder_prefix="%d" % (self.flags.epochs - 1))
                    activation = self.sess.run(self.net[layer][:,:,:,:], feed_dict={self.images: batch_images})
                else:
                    neighbor_search = self.train_neighbor_search(layer, channel, knn_param=knn_param,
                                                                folder_prefix="%d" % (self.flags.epochs - 1))
                    activation = self.sess.run(self.net[layer][:,:,:,channel], feed_dict={self.images: batch_images})
                ...
                for ii in trange(len(indices), desc="Processing test input for plot", leave=True, position=0):
                    ...
                    node_degree = np.sum(x_opt)
                    W = x_opt / np.sum(x_opt)
                    predicted_label = np.argmax(np.sum(np.expand_dims(W, axis=1) * self.train_dataset.labels[I[ii, :]], axis=0))
                    labels = y_train[non_zero]
                    weights = W[non_zero]
                    num_same_label = np.sum(labels == batch_labels[ii])
                    num_other_labels = no_neighbors - num_same_label
                    index_same_labels = np.where(labels == batch_labels[ii])[0].tolist()
                    weight_same_label = np.sum(weights[index_same_labels])
                    
                    # Compare with k-NN weights
                    W_knn = 1/D[ii, :] / np.sum(1/D[ii, :])
                    index_same_labels_knn = np.where(y_train == batch_labels[ii])[0].tolist()
                    weight_same_label_knn = np.sum(W_knn[index_same_labels_knn])
                    ...
                    for neighbor_itr in range(no_neighbors):
                        ...
                        ax.set_title("%s, %0.2e " % (self.class_labels[y_train[neighbor_itr]], W[neighbor_itr]),
                                    fontdict=fontdict)
                    output_name = "%s_query_%d_layer_%d_channel_%s_neighbors_%d" % (data_type, indices[ii], layer, str(channel), no_neighbors)
                    plt.savefig(
                        os.path.join(plot_results_path, output_name+'.png'),
                        bbox_inches='tight')
                    # .EPS FOR VECTOR IAMGES
                    plt.close()
                    
                    # stats_per_channel['channel_'+str(channel)+'_query_'+str(indices[ii])] = {
                    #     'query' : int(indices[ii]),
                    #     'label' : int(batch_labels[ii]),
                    #     'knn_param': int(knn_param),
                    #     'dimension': neighbor_search.d,
                    #     'num_neighbors': int(no_neighbors),
                    #     'node_degree': float(node_degree),
                    #     'neighbors': np.squeeze(I[ii, :])[non_zero].tolist(),
                    #     'weights': W[:no_neighbors].tolist(),
                    #     'labels': y_train[:no_neighbors].tolist(),
                    #     'predicted_label': int(predicted_label),    
                    # }
                    
                    with open(file_csv, 'a') as f_csv:
                        print(f'{int(indices[ii])},{int(layer)},{str(channel)},{int(no_neighbors)},{int(num_same_label)},{int(num_other_labels)},{weight_same_label},{weight_same_label_knn}', file=f_csv)
            
            # stats_per_layer['layer_'+str(layer)] = stats_per_channel
        
        # f_json.write(json.dumps(stats_per_layer, indent=4))    
        # f_json.close()
    ...
    def plot_activations(self, indices):
        data_type = self.flags.data_type
        if data_type == "train":
            test_dataset = self.train_dataset
        elif data_type == "test":
            ...
            raise EnvironmentError("Unknown calibration save data type: %s" % data_type)
        plot_activations_path = "plot_activations/"
        if not os.path.exists(plot_activations_path):
            os.makedirs(plot_activations_path)
        knn_layers = sorted(map(int, self.flags.knn_layers.split(",")))
        knn_channels = sorted(map(int, self.flags.knn_channels.split(",")))
        ...
        for ii in trange(len(indices), desc="Processing test input for plot", leave=True, position=0):
            ...
            fontdict = {'fontsize': 6}
            images_per_row = 16
            plt_rows = np.ceil(1.0 + (len(knn_layers)*len(knn_channels) / images_per_row)).astype(np.int)
            plt.figure(figsize=(6.4*2, plt_rows * 1.2))
            ax = plt.subplot(plt_rows, images_per_row, 8)
            plt.imshow(batch_images[ii] * self.train_std + self.train_mean, aspect='equal')
            ax.set_axis_off()
            ax.set_title("query %d, %s " % (indices[ii], self.class_labels[batch_labels[ii]]), fontdict=fontdict)
            
            for jj, layer in enumerate(knn_layers):  
                for kk, channel in enumerate(knn_channels):
                    activation = self.sess.run(self.net[layer][:,:,:,channel], feed_dict={self.images: batch_images})
                    
                    ax = plt.subplot(plt_rows, images_per_row, 16 + len(knn_channels)*(jj) + (kk+1))
                    plt.imshow(activation[ii] * self.train_std + self.train_mean, cmap='viridis', aspect='equal')
                    ax.set_axis_off()
                    ax.set_title("Layer %d, Ch %d" % (layer, channel), fontdict=fontdict)
            
            output_name = os.path.join(plot_activations_path, "activations_query_%d_layers_%s" % (indices[ii], str(knn_layers).replace('[','').replace(']','').replace(', ','_')))
            plt.savefig(os.path.join(output_name+'.eps'),bbox_inches='tight')
            # .EPS FOR VECTOR IAMGES
            plt.close()
            
    def plot_zeros_heatmap(self, indices):
        data_type = self.flags.data_type    
        if data_type == "train":
            test_dataset = self.train_dataset
        elif data_type == "test":
            ...
            raise EnvironmentError("Unknown calibration save data type: %s" % data_type)
        plot_zeros_heatmap_path = "plot_zeros_heatmap/"
        if not os.path.exists(plot_zeros_heatmap_path):
            os.makedirs(plot_zeros_heatmap_path)
        knn_layers = sorted(map(int, self.flags.knn_layers.split(",")))
        knn_channels = sorted(map(int, self.flags.knn_channels.split(",")))
        ...
        fontdict = {'fontsize': 6}
        images_per_row = len(knn_channels)
        plt_rows = np.ceil(len(indices) + (len(knn_layers)*len(knn_channels) / images_per_row)).astype(np.int)
        plt.figure(figsize=(25.6*2, plt_rows * 4.8))
        
        for ii in range(len(indices)):
            ax = plt.subplot(plt_rows, images_per_row, ii+1)
            plt.imshow(batch_images[ii] * self.train_std + self.train_mean, aspect='equal')
            ax.set_axis_off()
            ax.set_title("query %d, %s " % (indices[ii], self.class_labels[batch_labels[ii]]), fontdict=fontdict)
        ...
        for jj, layer in enumerate(knn_layers):  
            for kk, channel in enumerate(knn_channels):
                activation = self.sess.run(self.net[layer][:,:,:,channel], feed_dict={self.images: batch_images})
                zeros_heatmap = np.count_nonzero(activation, axis=0) / len(indices)
                
                ax = plt.subplot(plt_rows, images_per_row, images_per_row + len(knn_channels)*(jj) + (kk+1))
                plt.imshow(zeros_heatmap, cmap='jet', aspect='equal', vmin=0, vmax=1)
                ax.set_axis_off()
                ax.set_title("Layer %d, Ch %d" % (layer, channel), fontdict=fontdict)
        ...
        output_name = os.path.join(plot_zeros_heatmap_path, "plot_zeros_heatmap_queries_%s_layers_%s.png" % (str(indices).replace('[','').replace(']','').replace(', ','_'), str(knn_layers).replace('[','').replace(']','').replace(', ','_')))
