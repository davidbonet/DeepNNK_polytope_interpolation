...
flags.DEFINE_bool("separate_relu", False, "define the ReLU in a separate layer (the number of resulting layers is n_layers*2)")
flags.DEFINE_string("logs_dir", "logs/", "Path to logs dir")
flags.DEFINE_string("data_type", "test", "data_type")
...
flags.DEFINE_string("knn_channels", "1",
                    "Comma separated values corresponding to channels where KNN is performed in each knn_layer. For e.g 1,2")
...
def main(arg=None):
    ...
    if experiment == "neighbor":
        model = High_NNK(config=session_config, flags=FLAGS)
    else:
    ...
    if mode == "train":
        ...
        # model.plot_neighbors([2595]) # deer
        # model.plot_neighbors([374]) # bird
        # model.plot_neighbors([3745]) # car
    elif mode == "activations":
        # model.plot_activations([374])
        model.plot_activations([2595, 3745, 374])
    elif mode == "zeros_heatmap":
        model.plot_zeros_heatmap([2595, 3745, 374])
        # model.plot_zeros_heatmap([374]) # bird
        # model.plot_zeros_heatmap([2595, 3745, 374, 345, 268, 1899, 7643, 3457, 9999])
    else:
